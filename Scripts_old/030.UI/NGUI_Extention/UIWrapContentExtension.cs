...
public class UIWrapContentExtension : MonoBehaviour
{
    public delegate GameObject OnCreateItem();
    public delegate void OnUpdateItem(GameObject go, int wrapIndex, int realIndex);
    ...
    void CreateItemList()
    {
        ...
        if (onCreateItem != null)
        {
            ...
            if (mHorizontal)
            {
                itemCount = ((int)mPanel.width + itemSize - 1) / itemSize;
            }
            else
            {
                itemCount = ((int)mPanel.height + itemSize - 1) / itemSize;
            }

            for (int i = 0; i < itemCount * 2; i++)
            {
                GameObject go = onCreateItem();
                if (go != null)
                {
                    Utility.SetParent(mTrans, go.transform, true);
                }
            }
        }
    }
    ...
    public virtual void UpdateItemCount(int count)
    {
        int prevMinIndex = minIndex;
        minIndex = Mathf.Min(0, -(count - 1));
        maxIndex = 0;
