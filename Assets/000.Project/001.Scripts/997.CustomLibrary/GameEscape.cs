...
public class GameEscape
{
    ...
    private void EscapeDungeon()
    {
        if( CheckOkEscape() )
        {
            Transform tm = BKST.UISystemBK.Instance.FindPanel("Panel Pause");
    ...
    private void EscapePvpMatching()
    {
        if( CheckOkEscape() )
        {
            Transform tm = BKST.UISystemBK.Instance.FindPanel("Panel Lobby PVP");
            if(tm != null && tm.gameObject.activeSelf)
            {
    ...
    private bool CheckOkEscape()
    {
        if(Input.GetKeyDown(KeyCode.Escape))//일단 나가기 버튼을 눌러야 모든 검사 시작
        {
            ...
            if (BKST.UISystemBK.Instance.IsExistToHide()) return false;//UISystem.instance.Escape() 은 나가기 버튼 눌렀을때 더 이상 닫을 UI 없는가? 를 알아보는 메소드, 있다면 그걸 닫고 false, 없다면 다음줄 검사
